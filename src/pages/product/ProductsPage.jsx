...
const ProductsPage = () => {
  ...
  const fetchProducts = async () => {
    try {
      setLoading(true);
      const response = await axios.get(
        `${import.meta.env.VITE_BASE_URL || 'https://admin.fast2.in/proxy'}/api/product/get-products-admin?limit=1000`
      );
  ...
  const fetchProductStatus = async (productId) => {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_BASE_URL || 'https://admin.fast2.in/proxy'}/api/product/${productId}/status`
      );
  ...
  const toggleProductStatus = async (productId, currentStatus) => {
    try {
      ...
      const response = await axios.patch(
        `${import.meta.env.VITE_BASE_URL || 'https://admin.fast2.in/proxy'}/api/product/${productId}/toggle-active`
      );
  ...
  const fetchProductOrders = async (productId) => {
    try {
      ...
      const response = await axios.get(
        `${import.meta.env.VITE_BASE_URL || 'https://admin.fast2.in/proxy'}/api/product/${productId}/orders`
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.put(
        `${import.meta.env.VITE_BASE_URL || 'https://admin.fast2.in/proxy'}/api/product/${editingProduct._id}`,
        submitData,
        {
  ...
  const handleInlineUpdate = async (productId, field, value) => {
    try {
      ...
      const response = await axios.put(
        `${import.meta.env.VITE_BASE_URL || 'https://admin.fast2.in/proxy'}/api/product/${productId}`,
        { [backendField]: value },
  ...
  const handleDelete = async (productId, productName) => {
    ...
    if (window.confirm(`Are you sure you want to delete "${productName}"?`)) {
      try {
        await axios.delete(`${import.meta.env.VITE_BASE_URL || 'https://admin.fast2.in/proxy'}/api/product/${productId}`);
  ...
  const handleCSVUpload = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${import.meta.env.VITE_BASE_URL || 'https://admin.fast2.in/proxy'}/api/admin/products/upload/csv`,
        formData,
        {
          headers: {
  ...
  const downloadTemplate = async () => {
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_BASE_URL || 'https://admin.fast2.in/proxy'}/api/admin/products/download/template`,
        {
          headers: {
            'Authorization': `Bearer ${token}`
          }
                          ...
                          >
                            ...
                            >
                              <div
                                ...
                                onClick={() => setInlineEdit({ productId: product._id, field: 'quantity', value: product.quantity })}
                              >
                                {inlineEdit.productId === product._id && inlineEdit.field === 'quantity' ? (
                                  <input
                                    type="number"
                                    value={inlineEdit.value}
                                    onChange={(e) => setInlineEdit({ ...inlineEdit, value: e.target.value })}
                                    onBlur={() => handleInlineUpdate(product._id, 'quantity', inlineEdit.value)}
                                    onKeyDown={(e) => e.key === 'Enter' && handleInlineUpdate(product._id, 'quantity', inlineEdit.value)}
