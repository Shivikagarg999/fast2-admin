...
const OrdersPage = () => {
    ...
    const fetchOrders = async () => {
        try {
            ...
            const response = await fetch(`${import.meta.env.VITE_BASE_URL || 'http://localhost:5000'}/api/admin/orders/getall?${params.toString()}`);
    ...
    const handleStatusUpdate = async (orderId, newStatus) => {
        try {
            const response = await fetch(`${import.meta.env.VITE_BASE_URL || 'http://localhost:5000'}/api/admin/orders/${orderId}/status`, {
                method: 'PUT',
                headers: {
    ...
    const downloadCSV = async () => {
        try {
            ...
            const response = await fetch(`${import.meta.env.VITE_BASE_URL || 'http://localhost:5000'}/api/admin/orders/download/csv?${params.toString()}`);
    ...
    const handleDownloadInvoice = async (orderId) => {
        try {
            const response = await fetch(`${import.meta.env.VITE_BASE_URL || 'http://localhost:5000'}/api/order/${orderId}/invoice`);
