...
const UsersPage = () => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      const res = await axios.get(
        `${import.meta.env.VITE_BASE_URL || 'https://admin.fast2.in/proxy'}/api/admin/users`,
  ...
  const handleEditUser = async () => {
    ...
    try {
      ...
      const res = await axios.put(
        `${import.meta.env.VITE_BASE_URL || 'https://admin.fast2.in/proxy'}/api/admin/users/${selectedUser._id}`,
        updateData,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handleDeleteUser = async (userId, userName) => {
    ...
    try {
      ...
      const res = await axios.delete(
        `${import.meta.env.VITE_BASE_URL || 'https://admin.fast2.in/proxy'}/api/admin/users/${userId}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handleAddMoney = async () => {
    ...
    try {
      ...
      const token = localStorage.getItem("token");
      const res = await axios.post(
        `${import.meta.env.VITE_BASE_URL || 'https://admin.fast2.in/proxy'}/api/admin/users/${selectedUser._id}/wallet/add`,
        {
  ...
  const downloadCSV = async () => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_BASE_URL || 'https://admin.fast2.in/proxy'}/api/admin/users/download/csv?${params.toString()}`);
