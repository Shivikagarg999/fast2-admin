...
const EditPromotorPage = () => {
  ...
  useEffect(() => {
    const fetchPromotor = async () => {
      try {
        setFetching(true);
        const response = await axios.get(`${import.meta.env.VITE_BASE_URL || 'https://admin.fast2.in/proxy'}/api/admin/promotor/${id}`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(`${import.meta.env.VITE_BASE_URL || 'https://admin.fast2.in/proxy'}/api/admin/promotor/${id}`, formData);
